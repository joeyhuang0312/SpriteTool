...
try {
  for (const plistFile of plistFiles) {
    ...
    console.log(`relativePath:${relativePath},plistName:${plistName},dirPath:${dirPath}`);

    const currentPlistPath = plistFile;
    let currentImagePath = path.join(path.dirname(plistFile), `${plistName}.png`);
    if (!fs.existsSync(currentImagePath)) 
        currentImagePath=path.join(path.dirname(plistFile), `${plistName}.jpg`);
